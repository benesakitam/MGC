...
def main():
    ...
    file_name = os.path.join(pretrained_file_name, "{}_linear_eval".format(exp.save_folder_prefix))

    if args.local_rank == 0:
        if not os.path.exists(file_name):
            os.makedirs(file_name, exist_ok=True)

    logger = setup_logger(file_name, distributed_rank=args.local_rank, filename="aid.txt", mode="a")
    if args.local_rank == 0:
    ...
    state_dict_new = {k.replace("transdown_layer", "mlps"): v for k, v in state_dict.items()}
...
if __name__ == '__main__':
    ...
    parser.add_argument("-expn", "--experiment-name", type=str, default="baseline-")
    parser.add_argument("-ckpt", "--checkpoints", type=str, default="100")
    ...
    from exps.linear_eval_exp import Exp
    exp = Exp(args)
